...
const OPERATOR_EMAIL = "ashton@directfreightco.com";
...
const LAUNCH_DATE = new Date("2020-01-01T00:00:00Z"); // TEST MODE — unlocked
const IS_LAUNCHED = Date.now() >= LAUNCH_DATE.getTime();

// ── TRIAL SETTINGS ───────────────────────────────────────────────────
const TRIAL_DAYS = 30; // In test mode set to 0 to test paywall: const TRIAL_DAYS = 0;
...
export default function HaulBoard() {
  ...
  const startEmailVerification = async (email, name, sessionToSet) => {
    // 🧪 TEST MODE — email verification skipped entirely, log straight in
          ...
          onIndieShipperAuth={async (s) => {
            try {
              ...
              try {
                ...
                // 🧪 TEST MODE — backend unreachable, create account locally so testing isn't blocked
                console.warn("Backend unreachable — creating local test account:", e.message);
                const localUser = { ...s, id: "local_" + Date.now(), role: "shipper", ratings: [] };
                setIndependentShippers((p) => upsert(p, localUser));
                startEmailVerification(localUser.email, localUser.name, { role: "shipper", corpId: null, profileId: localUser.id });
              }
            }
          }}
          onIndieTruckerAuth={async (t) => {
            try {
              ...
              try {
                ...
                // 🧪 TEST MODE — backend unreachable, create account locally so testing isn't blocked
                console.warn("Backend unreachable — creating local test account:", e.message);
                const localUser = { ...t, id: "local_" + Date.now(), role: "trucker", ratings: [] };
                setIndependentTruckers((p) => upsert(p, localUser));
                startEmailVerification(localUser.email, localUser.name, { role: "trucker", corpId: null, profileId: localUser.id });
              }
            }
          }}
...
function AboutModal({ onClose, onJoinWaitlist }) {
  const SUPPORT_EMAIL = "ashton@directfreightco.com";
  return (
...
const OPERATOR_NOTIFY_EMAIL = "ashton@directfreightco.com";
...
function OnboardingProgressMap({ me, role }) {
  const steps = role === "trucker" || role === "trucking" ? [
    ...
    { key: "eld", label: "ELD connected", done: !!me.eld?.connected },
  ] : [
    ...
    <div id="subscription-panel" style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 520 }}>
      ...
      <Card style={{ padding: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <a href="mailto:ashton@directfreightco.com?subject=Update payment method" style={{ ...ghostBtn, justifyContent: "flex-start", display: "flex", gap: 6, alignItems: "center", textDecoration: "none", color: "inherit" }}>
            <CreditCard size={14} /> Update payment method
          </a>
          <a href="mailto:ashton@directfreightco.com?subject=Invoice request" style={{ ...ghostBtn, justifyContent: "flex-start", display: "flex", gap: 6, alignItems: "center", textDecoration: "none", color: "inherit" }}>
            <FileText size={14} /> Request invoices
          </a>
          <a href="mailto:ashton@directfreightco.com?subject=Cancellation request" style={{ ...ghostBtn, color: "#FF5A1F", justifyContent: "flex-start", display: "flex", gap: 6, alignItems: "center", textDecoration: "none" }}>
            <X size={14} /> Cancel subscription
          </a>
        </div>
      </Card>
      <div style={{ fontSize: 11, color: "#9A958A", lineHeight: 1.5 }}>
        To update your payment method, email <b>ashton@directfreightco.com</b>. Cancellation takes effect at the end of your current billing period. No pro-rated refunds except where required by law.
      </div>
      {!isComplimentary && me && (
...
function CorpLoginForm({ subtype, existing, onBack, onSubmit, embedded, selectedTier }) {
  ...
  const trySignup = async () => {
    ...
    if (!agreedToTerms) { alert("You must agree to the Terms of Service to create an account."); return; }
    // TEST MODE: business doc, DOT/MC, COI, and Stripe checks bypassed for testing
    ...
    try {
      ...
      try {
        ...
        // 🧪 TEST MODE — backend unreachable, create account locally so testing isn't blocked
        console.warn("Backend unreachable — creating local test company account:", e.message);
        onSubmit({
          id: "local_" + Date.now(),
          companyName, subtype, email,
          mcNumber: onboarding?.data?.mcNumber || null,
          dotNumber: onboarding?.data?.dotNumber || null,
          verification: onboarding?.data || null,
          members: [],
...
function PrivacyModal({ onClose }) {
  return (
    <LegalModal title="Privacy Policy" icon={<FileText size={16} color="#FFB400" />} onClose={onClose}>
      ...
      <h4 style={termsH4}>2. Location tracking consent</h4>
      <p><b>Carriers explicitly consent to continuous real-time location tracking while a load is active.</b> Your current ZIP, GPS coordinates (where available from your ELD), and Loaded/Empty status are shared with the Shipper on your active load and with other Shippers using the Nearby Capacity radar. This is required for the Platform to function and cannot be opted out of during an active haul.</p>
...
function QuickPostForm({ onSubmit }) {
  ...
  const submit = () => {
    ...
    onSubmit({
      ...
      requirements: { minSafetyRating: "any", requireMcVerified: false, requireGpsTracking, minCarrierRating: 0, preferences: "" },
    });
...
// Simulated live GPS — interpolates a position between origin/destination ZIP
// centroids based on load.progress. A real version would read this position
// from a driver-side GPS feed (ELD integration or mobile app), not from %complete.
  ...
  return (
    <Card style={{ padding: 20 }}>
      ...
      <div style={{ fontSize: 13, color: "#6B6557", marginBottom: 14, lineHeight: 1.5 }}>
        Mileage from your completed hauls on Direct Freight. Export for quarterly IFTA fuel tax filing. Connect your ELD for automatic state-by-state breakdown.
      </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(27,29,33,0.97)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 24, overflowY: "auto" }}>
      <div style={{ width: "100%", maxWidth: 680, color: "#F2EDE4", fontFamily: "Inter, sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", justifyContent: "center", marginBottom: 24 }}>
          ...
          {(me?.role === "trucker" || me?.role === "shipper") && (
            <div style={{ background: "#1B1D21", border: "2px solid #FFB400", borderRadius: 12, padding: 24, flex: 1, minWidth: 200, maxWidth: 260 }}>
              ...
              <a href="mailto:ashton@directfreightco.com?subject=Subscribe - {me.role}" style={{ display: "block", background: "#FFB400", color: "#1B1D21", borderRadius: 8, padding: "12px 0", fontWeight: 700, fontSize: 14, textAlign: "center", textDecoration: "none" }}>
                Subscribe now
              </a>
            </div>
          )}
          ...
          {me?.role !== "trucker" && me?.role !== "shipper" && COMPANY_TIERS.map(t => (
            <div key={t.id} style={{ background: "#1B1D21", border: "1px solid #2A2D32", borderRadius: 12, padding: 18, flex: 1, minWidth: 150 }}>
              ...
              <a href={`mailto:ashton@directfreightco.com?subject=Subscribe - ${t.name}`} style={{ display: "block", background: "#2A2D32", color: "#FFB400", borderRadius: 6, padding: "8px 0", fontWeight: 700, fontSize: 12, textAlign: "center", textDecoration: "none" }}>
                Subscribe
              </a>
            </div>
          ))}
        ...
        <div style={{ textAlign: "center", display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
          <div style={{ fontSize: 12, color: "#6B6557" }}>
            To subscribe, email <a href="mailto:ashton@directfreightco.com" style={{ color: "#FFB400" }}>ashton@directfreightco.com</a> or contact us to get set up. Stripe payments coming soon.
          </div>
        ...
        {status === "success" && result ? (
          <div style={{ background: "#2A2D32", border: "2px solid #FFB400", borderRadius: 14, padding: 28, textAlign: "left" }}>
            ...
            <div style={{ fontSize: 12, color: "#6B6557", lineHeight: 1.6, textAlign: "center" }}>
              ...
              Questions? Email us at <a href="mailto:ashton@directfreightco.com" style={{ color: "#FFB400" }}>ashton@directfreightco.com</a>
            </div>
